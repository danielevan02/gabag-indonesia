...
const CartItem = ({ item, cartItemsLength, index, startTransition }: CartItemProps) => {
  ...
  const debouncedUpdate = useRef(
    debounce((productId: string, finalQty: number, variantId?: string) => {
      startTransition(async () => {
        const res = await updateCartItem.mutateAsync({ productId, quantity: finalQty, variantId });
  ...
  const handleDeleteQty = async () => {
    await deleteCartItem.mutateAsync({ productId: item.productId, variantId: item.variantId });
  };
