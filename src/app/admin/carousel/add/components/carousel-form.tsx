...
export default function CarouselForm() {
  ...
  const { isPending, mutateAsync } = trpc.carousel.create.useMutation({
    onSuccess: (resp) => {
      if (resp.success) {
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        <div className="flex flex-col gap-4 my-5 rounded-md border w-fit p-3">
          <Label className="text-muted-foreground">This will make the carousel visible</Label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <FormInput
            description="Determine when will the carousel active"
            form={form}
            fieldType="date"
          ...
          />
          <FormInput
            description="Determine when will the carousel inactive"
            form={form}
            fieldType="date"
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={isPending}>
            {isPending ? <Loader className="w-4 h-4 animate-spin" /> : "Create Carousel"}
          </Button>
        </div>
      </form>
    </Form>
