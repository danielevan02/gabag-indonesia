...
// Flattened item representing either a product or a variant
interface CampaignItem {
  productId: string;
  variantId?: string;
  displayName: string; // "Product Name" or "Product Name - Variant Name"
  regularPrice: number;
...
export default function CampaignForm({ products }: CampaignFormProps) {
  ...
  const flattenedItems: Array<{
    ...
    if (product.hasVariant && product.variants && product.variants.length > 0) {
      // Product has variants - show each variant as separate item
  ...
  // Sync selectedItems dengan form
      ...
      >
        ...
        <div className="space-y-4 border-b pb-6">
          ...
          <FormDescription className="text-xs text-muted-foreground">
            💡 Tip: For default/permanent campaign, only set Start Date
          </FormDescription>
        </div>
        ...
        {/* Item Selection (Products & Variants) */}
        <div className="space-y-4 border-b pb-6">
        ...
        <div className="flex justify-end gap-2 sticky bottom-0 inset-x-0 bg-background pt-4 pb-2">
          ...
          <Button type="submit" disabled={isLoading || selectedItems.length === 0}>
            {isLoading ? <Loader className="w-4 h-4 animate-spin" /> : "Create Campaign"}
          </Button>
        </div>
      </form>
    </Form>
