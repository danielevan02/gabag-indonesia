...
export const VariantImageField = ({ form, fieldName, allMediaFiles }: VariantImageFieldProps) => {
...
const VariantImageSection = ({
  ...
  return (
    <div className="flex gap-2 flex-col mb-5">
      <VariantImageLabel isError={isImageError} />
      <VariantImageContent
        hasImage={hasImage}
        imageId={field.value}
        isError={isImageError}
      />
      <VariantImageError isError={isImageError} errorMessage={errorMessage} />
...
const VariantImageLabel = ({ isError }: { isError: boolean }) => (
  <>
...
interface VariantImageContentProps {
  hasImage: boolean;
  imageId?: string;
  isError: boolean;
}

const VariantImageContent = ({ hasImage, imageId, isError }: VariantImageContentProps) => {
  if (hasImage && imageId) {
    return <VariantImageDisplay imageId={imageId} />;
  }

  return <VariantImagePlaceholder isError={isError} />;
};

// 5. Component untuk Display Image
const VariantImageDisplay = ({ imageId }: { imageId: string }) => {
...
const VariantImagePlaceholder = ({ isError }: { isError: boolean }) => {
...
const VariantImageError = ({ isError, errorMessage }: { isError: boolean, errorMessage?: string }) => {
...
interface VariantImageGalleryModalProps {
  field: ControllerRenderProps<any, string>;
  allMediaFiles?: {
    images: {
      id: string;
...
const VariantImageGalleryModal = ({
  field,
  allMediaFiles,
}: VariantImageGalleryModalProps) => {
