    ...
    <div className="flex flex-col px-5 w-full max-w-screen">
      ...
      <div className="relative flex flex-col md:flex-row md:gap-5 justify-center items-start w-full">
        ...
        <div className="mt-5 max-w-80 xl:max-w-96">
          <h2 className="uppercase tracking-wider text-foreground/60 text-sm md:text-base">{product?.categories[0].name}</h2>
          <h1 className="md:text-xl font-medium tracking-wider mb-5">{product?.name}</h1>
          <PriceTag price={Number(product?.price)} discount={product?.discount || categoryDiscount}/>
          ...
          <div className="flex flex-col gap-3 mt-5 mb-10">
            <p className="">Stock: <span className="font-medium">{product?.stock}</span></p>
            ...
            <Button className="uppercase tracking-widest rounded-full py-7 w-full mt-5">add to cart</Button>
            <Button className="uppercase tracking-widest rounded-full py-7 w-full" variant='outline'>favourite</Button>
          </div>

          <pre className="whitespace-pre-wrap text-neutral-700" style={{fontFamily: 'inherit'}}>{product?.description}</pre>
    ...
    <>
      {discount ? (
        <div className="flex flex-row gap-3 mb-5 items-center">
          <h3 className="font-semibold text-lg tracking-wider">Rp {lastPrice.toLocaleString()}</h3>
